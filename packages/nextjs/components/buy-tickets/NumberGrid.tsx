      ...
      {Array.from({ length: 41 }).map((_, numIdx) => {
        ...
        const isLuckElement = num === 0;
          ...
          <motion.button
            ...
            custom={numIdx}
            initial="hidden"
            whileHover={isLuckElement || (selectedNumbers?.length >= 5 && !isSelected) ? {} : { scale: 1.1 }}
            whileTap={isLuckElement || (selectedNumbers?.length >= 5 && !isSelected) ? {} : { scale: 0.9 }}
            onClick={() => onNumberSelect(ticketId, num)}
            data-ticket={ticketId}
            data-number={num}
            disabled={isLuckElement || (selectedNumbers?.length >= 5 && !isSelected)}
            animate={
              isLuckElement ? "animate" :
            ...
            variants={isLuckElement ? luckAnimationVariants : numberAnimationVariants}
          ...
          >
            {isLuckElement ? (
              <>
                ...
                <span className="text-xs font-bold text-white drop-shadow-sm relative z-10">LUCK</span>
              </>
            ) : (
              num < 10 ? `0${num}` : num
            )}
