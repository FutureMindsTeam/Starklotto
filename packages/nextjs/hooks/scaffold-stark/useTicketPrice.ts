...
export function useTicketPrice(opts?: { contractName?: string; decimals?: number; watch?: boolean }) {
  const contractName = opts?.contractName ?? LOTT_CONTRACT_NAME ?? "Lottery";
  const decimals = opts?.decimals ?? 18;
  const watch = opts?.watch ?? true;

  const {   data,   isLoading,   isFetching,  error, refetch, } = useScaffoldReadContract({
  ...
  const priceWei: bigint = useMemo(() => {
    ...
    if (typeof value === "object" && "low" in value && "high" in value) {
      return (BigInt((value as any).high ?? 0) << 128n) + BigInt((value as any).low ?? 0);
    }
    try {
