use contracts::Lottery::{
    ILotteryDispatcher, ILotteryDispatcherTrait, ILotterySafeDispatcher,
    ILotterySafeDispatcherTrait, Lottery,
};
...
use snforge_std::{
    ...
    spy_events, start_cheat_block_timestamp, start_cheat_caller_address, stop_cheat_caller_address,
};
...
fn test_multiple_tickets_same_user() {
    ...
    lottery_dispatcher
        .BuyTicket(1, create_single_ticket_numbers_array(create_another_valid_numbers()), 1);
    stop_cheat_caller_address(lottery_address);
...
fn test_tickets_across_different_draws() {
    ...
    start_cheat_caller_address(lottery_address, user1_address());
    lottery_dispatcher
        .BuyTicket(2, create_single_ticket_numbers_array(create_another_valid_numbers()), 1);
    stop_cheat_caller_address(lottery_address);
...
fn test_buy_multiple_tickets_with_custom_numbers() {
    ...
    ticket1_numbers.append(1);
    ticket1_numbers.append(2);
    ticket1_numbers.append(3);
    ticket1_numbers.append(4);
    ticket1_numbers.append(5);
    numbers_array.append(ticket1_numbers);

    // Ticket 2: [10, 20, 30, 35, 40]
    let mut ticket2_numbers = ArrayTrait::new();
    ticket2_numbers.append(10);
    ticket2_numbers.append(20);
    ticket2_numbers.append(30);
    ticket2_numbers.append(35);
    ticket2_numbers.append(40);
...
fn test_ticket_purchase_token_transfer() {
    ...
    assert(
        erc20_dispatcher.balance_of(user1_address()) == INITIAL_USER_BALANCE,
        'pre-buy balance mismatch',
    );
    assert(erc20_dispatcher.balance_of(vault_address) == 0_u256, 'balance pre-buy not zero');
    assert(
        erc20_dispatcher.allowance(user1_address(), lottery_address) == INITIAL_ALLOWANCE,
        'Allowance pre-buy mismatch',
    );
    ...
    assert(
        erc20_dispatcher.balance_of(user1_address()) == POST_PURCHASE_BALANCE,
        'post-buy balance mismatch',
    );
    assert(erc20_dispatcher.balance_of(vault_address) == TICKET_PRICE, 'post-buy balance mismatch');
    assert(
        erc20_dispatcher.allowance(user1_address(), lottery_address) == POST_PURCHASE_ALLOWANCE,
        'Allowance post-buy mismatch',
    );
}
...
fn test_buy_token_insufficient_user_balance() {
    ...
    start_cheat_caller_address(token_address, user1_address());
    erc20_dispatcher
        .transfer(user2_address(), (erc20_dispatcher.balance_of(user1_address()) - 1_u256));
    stop_cheat_caller_address(token_address);
...
fn test_vault_balance_accumulated_prize() {
    ...
    assert(
        erc20_dispatcher.balance_of(vault_address) == INITIAL_VAULT_BALANCE,
        'vault balance not zero',
    );
    ...
    assert(
        erc20_dispatcher.balance_of(vault_address) == SINGLE_TICKET_VAULT_BALANCE,
        'Vault balance after one',
    );
    ...
    assert(
        erc20_dispatcher.balance_of(vault_address) == TWO_TICKET_VAULT_BALANCE,
        'Vault balance did not',
    );
}
...
fn test_buy_ticket_insufficient_lottery_allowance() {
    ...
    assert(
        erc20_dispatcher.allowance(user_address, lottery_address) == INITIAL_USER_BALANCE,
        'pre allowance mismatch',
    );
    ...
    assert(
        erc20_dispatcher.allowance(user_address, lottery_address) == POST_PURCHASE_ALLOWANCE,
                ...
                (
                    ...
                    ERC20Component::Event::Transfer(
                        ERC20Component::Transfer {
                            from: user1_address(), to: vault_address, value: TICKET_PRICE,
                        },
                    ),
                ),
            ],
...
fn test_buy_ticket_failed_transfer_no_state_change() {
    ...
    assert(
        erc20_dispatcher.allowance(user1_address(), lottery_address) == INITIAL_USER_BALANCE,
        'pre allowance mismatch',
    );
    ...
    // Set a new, lower user balance to simulate an insufficient funds scenario.
    start_cheat_caller_address(lottery_address, user1_address());
    match lottery_dispatcher
        .BuyTicket(1, create_single_ticket_numbers_array(create_valid_numbers()), 1) {
...
fn test_multiple_tickets_token_transfer() {
    ...
    // Assert the initial user and vault balances are as expected.
    assert(
        erc20_dispatcher.balance_of(user_address) == INITIAL_USER_BALANCE,
        'pre-buy user balance mismatch',
    );
    ...
    assert(
        erc20_dispatcher.balance_of(user_address) == EXPECTED_USER_BALANCE_AFTER,
        'user balance not decreased',
    );

    // Verify vault balance increased by exactly the total price for 3 tickets
    assert(
        erc20_dispatcher.balance_of(vault_address) == EXPECTED_VAULT_BALANCE_AFTER,
        'vault balance not increased',
    );
